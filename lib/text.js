...
// Move cursor to the position where it ends up after applying the operation.
var applyOpToCursor = function(cursor, op) {
    var pos = 0;
    for (var i = 0; i < op.length; i++) {
        var c = op[i];
        switch (typeof c) {
...
exports.semanticInvert = function (str, op) {
    ...
    for (var i = op.length - 1; i > -1; i--) {
        ...
        case 'string':
            ...
            // and move the cursor if needed
            var delOp = {d: c.length};

            var cursor = applyOpToCursor(0, op.slice(0, i).concat(delOp));
            if (cursor > 0) inverseOps.push(cursor);

            inverseOps.push(delOp);
            break;
        case 'object':
            // In case of a deletion we need to reinsert the deleted string
            inverseOps.push(str.slice(0, c.d));
            break;
        }
    }

    return trim(inverseOps);
};
