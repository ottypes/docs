...
json.filterSnapshot = function(data, fields) {
  // We only want to keep fields on the snapshot that match specified fields.
  // Filtering only makes sense on json type objects who's data is a proper object,
  // we return null for all other types (number, string, array, null)
  ...
  for (var i = 0; i < fields.length; i++) {
    field = fields[i];
    val = data[field];
    if(val) newData[field] = val;
  }
  return newData;
}

json.filterOp = function(op, fields) {
  var newOp = [];
  for (var i = 0; i < op.length; i++) {
    var c = op[i];
    if(c.p.length >=1) {
      // look at the first element in the path list to see if it matches one of our fields.
      var root = c.p[0];
      if(~fields.indexOf(root)) {
        newOp.push(c);
      }
    } else {
      // the whole doc is being set/del, so filter the object its being set/deleted to/from.
      if(c.oi !== void 0) {
        c.oi = json.filterSnapshot(c.oi, fields);
      }
      if(c.od !== void 0) {
        c.od = json.filterSnapshot(c.od, fields);
      }
      newOp.push(c);
    }
  }
