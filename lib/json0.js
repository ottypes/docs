...
json.invertComponent = function(c) {
  ...
  for (var i = 0; i < subtypes.length; i++) {
    ...
    if (subtype.supportsOp(c)) {
      return subtype.invert(c);
    }
  }
...
json.apply = function(snapshot, op) {
  ...
  outer: for (var i = 0; i < op.length; i++) {
    var c = op[i];
    ...
    for (var k = 0; k < subtypes.length; k++) {
      ...
      if (subtype.supportsOp(c)) {
        parent[parentKey] = subtype.apply(elem, subtype.convertTo(c));
        continue outer;
      }
    }
...
json.append = function(dest,c) {
  ...
  if (dest.length != 0 && pathMatches(c.p, (last = dest[dest.length - 1]).p)) {
    ...
    for (var i = 0; i < subtypes.length; i++) {
      var subtype = subtypes[i];
      if (subtype.append && subtype.supportsOp(c) && subtype.supportsOp(last)) {
        var ops = [subtype.convertTo(last)];
        subtype.append(ops, subtype.convertTo(c));
  
        if (ops.length !== 1) {
          dest.push(c);
        } else {
          subtype.convertFrom(last, ops[0]);
        }
...
json.transformComponent = function(dest, c, otherC, type) {
  ...
  if (common != null) {
    ...
    for (var i = 0; i < subtypes.length; i++) {
      var subtype = subtypes[i];
      if (subtype.supportsOp(c) && subtype.supportsOp(otherC)) {
        var res = subtype.transform(subtype.convertTo(c), subtype.convertTo(otherC), type);
        
        // convert transformed components back to json components
        for (var i = 0; i < res.length; i++) {
          var jc = subtype.convertFrom({p: c.p}, res[i]);
          json.append(dest, jc);
        }
...
json.registerSubtype({
  // check the given operation is a text op
  supportsOp: function(op) {
    return op.si != null || op.sd != null;
  },
  
  // converts a json op to a text op
  convertTo: function(jc) {
    var tc = {p: jc.p[jc.p.length - 1]};
    if (jc.si != null) tc.i = jc.si;
    if (jc.sd != null) tc.d = jc.sd;
    return tc;
  },
  
  // convert a text op to a json op
  convertFrom: function(jc, tc) {
    jc.p[jc.p.length - 1] = tc.p;
  ...
  // apply a text op
  apply: function(snapshot, c) {
    return text.apply(snapshot, [c]);
  },
  
  // append a text op
  append: function(dest, c) {
    text._append(dest, c);
  },
  
  // transform a text op
  transform: function(c1, c2, type) {
    return text._tc([], c1, c2, type);
  }
});
